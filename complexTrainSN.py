...
class Trainer:
    def __init__(self, sample_rate, generator, discriminator, classifier,
        ...
        self.g_opt = optim.Adam(self.G.parameters(), lr=self.lr*5, betas=(0.5, 0.9))
        self.d_opt = optim.Adam(self.D.parameters(), lr=self.lr, betas=(0.5, 0.9))
    ...
    def inception_score(self, num_batch=10, splits=1):
        ...
        r, i = self.G(torch.FloatTensor(noise_r[0:1]).cuda(self.gpu),
                      torch.FloatTensor(noise_i[0:1]).cuda(self.gpu))
    ...
    def train(self):
        ...
        with open("./train/SN_losses.txt", 'w') as f:
...
if __name__ == '__main__':
    ...
    from complexSngan import *
    if load:
        g = ComplexGenerator()
        d = ComplexDiscriminator()
        checkpoint = torch.load('checkpoint_SN_complex.pth')
    ...
    trainer = Trainer(sample_rate=16000, generator=g, discriminator=d, classifier=c,
                      critic=1, batch_size=16, gpu=0, it=iteration)
    trainer.train()
