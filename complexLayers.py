...
from torch.nn import ConvTranspose2d, AdaptiveAvgPool2d
from complexFunctions import complex_relu, complex_tanh, complex_max_pool2d
...
class ComplexAdaptiveAvgPool2d(Module):
    def __init__(self, output_size):
        ...
        self.AdaptiveAvgPool = AdaptiveAvgPool2d(output_size=output_size)

    def forward(self, input_r, input_t):
        return self.AdaptiveAvgPool(input_r), self.AdaptiveAvgPool(input_t)
...
class ComplexTanh(Module):
    ...
    def forward(self, input_r, input_i):
        return complex_tanh(input_r, input_i)
...
class SpectralNormComplexConv2d(Module):

    def __init__(self,in_channels, out_channels, kernel_size=3, stride=1, padding = 0,
                 dilation=1, groups=1, bias=True):
        super(SpectralNormComplexConv2d, self).__init__()
        self.conv_r = Conv2d(in_channels, out_channels, kernel_size, stride, padding, dilation, groups, bias)
        self.conv_i = Conv2d(in_channels, out_channels, kernel_size, stride, padding, dilation, groups, bias)
        self.snconv_r = spectral_norm(self.conv_r)
        self.snconv_i = spectral_norm(self.conv_i)

    def forward(self,input_r, input_i):
        #  assert(input_r.size() == input_i.size())
        return self.snconv_r(input_r)-self.conv_i(input_i), \
               self.snconv_r(input_i)+self.conv_i(input_r)
...
class SpectralNormComplexLinear(Module):

    def __init__(self, in_features, out_features):
        super(SpectralNormComplexLinear, self).__init__()
        self.fc_r = spectral_norm(Linear(in_features, out_features))
        self.fc_i = spectral_norm(Linear(in_features, out_features))
